...
async function generarCapturaDevToolsDespues() {
  ...
  const html = `
    ...
    <head>
      ...
      <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .inspect-icon { color: #8ab4f8; font-weight: bold; font-size: 14px; }
        .tab { color: #9aa0a6; padding: 6px 8px; font-size: 11px; border-bottom: 2px solid transparent; }
        .tab.active { color: #e8eaed; border-bottom: 2px solid #8ab4f8; }
        .main-pane { flex: 1; display: grid; grid-template-columns: 1.1fr 1fr; overflow: hidden; }
        .elements-pane {
          background: #202124;
        ...
        .computed-pane { background: #242528; display: flex; flex-direction: column; overflow-y: auto; }
        .computed-header {
          background: #292a2d;
          border-bottom: 1px solid #3c4043;
          display: flex;
        ...
        .computed-subtab { font-size: 11px; color: #9aa0a6; padding: 4px 6px; }
        .computed-subtab.active { color: #e8eaed; border-bottom: 2px solid #8ab4f8; }
        .filter-bar { padding: 6px 12px; border-bottom: 1px solid #3c4043; background: #202124; }
        .filter-input {
          background: #292a2d;
        ...
        .prop-row { display: flex; justify-content: space-between; padding: 1px 0; }
        .prop-name { color: #9aa0a6; }
        .prop-val { color: #e8eaed; }
        .prop-val.highlight { color: #8ab4f8; font-weight: bold; }
        .rendered-fonts-section { padding: 12px; background: #202124; }
        .section-title {
          font-weight: bold;
          font-size: 11px;
          color: #e8eaed;
        ...
        .font-name { font-size: 13px; color: #81c995; font-weight: 600; margin-bottom: 4px; }
        .font-meta { color: #9aa0a6; font-size: 11px; line-height: 1.5; }
        .preview-banner {
          background: #18191a;
          border-bottom: 1px solid #3c4043;
          padding: 12px 20px;
    ...
    <body>
      <div class="preview-banner">
        <div>
          <span style="color: #9aa0a6; font-size: 11px; text-transform: uppercase;">Elemento Inspeccionado en P-18 (DESPUÉS DE CORRECCIÓN):</span>
          <div style="margin-top: 4px;">
            <code style="color: #81c995; font-size: 12px;">div.tarjeta-dato-valor</code>
            <span style="color: #666; margin: 0 8px;">|</span>
            <span style="color: #81c995; font-size: 11px;">✅ R y A completas · Tipografía designada para web: Caviar Dreams</span>
          </div>
        </div>
        <div class="inspected-target">
          <div class="titular-render">Pack Empresarial</div>
        </div>
      </div>
        ...
        <div class="computed-pane">
          ...
          <div class="computed-list">
            ...
            <div class="prop-row">
              ...
              <span class="prop-val highlight">"Caviar Dreams", system-ui, sans-serif</span>
            </div>
            <div class="prop-row">
          ...
          <div class="rendered-fonts-section">
            ...
            <div class="font-card">
              <div class="font-name">Caviar Dreams</div>
              <div class="font-meta">Network resource (16 glyphs)</div>
              <div class="font-meta" style="margin-top: 4px; color: #81c995;">
                PostScript name: <code>CaviarDreams</code>
              </div>
              <div class="font-meta" style="margin-top: 6px; font-size: 10px; color: #81c995;">
                * Resultado: R y A completas en mayúsculas y minúsculas. Tipografía con 4 pesos completos oficial para aplicaciones web según Brandbook.
              </div>
            </div>
          </div>
        </div>
  ...
  await page.screenshot({
    path: `${ARTIFACTS_DIR}/captura-devtools-computed-despues.png`
  });
  await browser.close();
  console.log('Captura DevTools DESPUÉS generada con éxito.');
}

generarCapturaDevToolsDespues().catch(console.error);
